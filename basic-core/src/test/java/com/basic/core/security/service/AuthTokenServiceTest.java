...
class AuthTokenServiceTest {
    ...
    @Test
    void savesLoginSessionUsingProvidedExpireMillis() {
        LoginSession session = buildSession(10L, "token-value");

        authTokenService.saveLoginSession(session, 21_600_000L);

        verify(redisUtils).set(eq("login:session:10"), org.mockito.ArgumentMatchers.any(String.class),
                eq(21_600_000L), eq(TimeUnit.MILLISECONDS));
    }

    @Test
    void rejectsNonPositiveLoginSessionExpiration() {
        org.assertj.core.api.Assertions.assertThatThrownBy(
                        () -> authTokenService.saveLoginSession(buildSession(10L, "token-value"), 0))
