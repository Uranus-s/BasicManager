  ...
  <el-dropdown @command="handleCommand" trigger="click">
    ...
    >
      ...
      <div class="user-info">
        ...
        <div class="user-role">{{ deptLabel }}</div>
      </div>
      <!-- 直接使用图标组件 -->
      ...
      <el-dropdown-menu class="custom-dropdown">
        <div class="dropdown-header">
          ...
          <div class="header-info">
            ...
            <div class="header-email">{{ emailLabel }}</div>
          </div>
        </div>
...
const deptNames = computed(() => store.getters["user/deptNames"] || []);
const displayName = computed(() => nickname.value || username.value || "未命名用户");
const emailLabel = computed(() => email.value || "未绑定邮箱");
const deptLabel = computed(() => {
  if (!deptNames.value.length) return "未配置部门";
  return deptNames.value.join("、");
});
