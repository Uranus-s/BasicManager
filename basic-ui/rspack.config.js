...
// 设置环境变量
process.env.VUE_APP_TITLE = title || "BasicManage";
process.env.VUE_APP_UPDATE_TIME = time;
process.env.BASE_URL = publicPath;
// 删除这一行，避免覆盖rspack.js中设置的值
// process.env.NODE_ENV = process.env.NODE_ENV || 'development'
process.env.VUE_APP_AUTHOR = "BasicManage"; // 设置作者
...
module.exports = {
  ...
  devServer: {
    ...
    setupMiddlewares: (middlewares, devServer) => {
      ...
      const registerBackendProxy = (prefix) => {
        const http = require("http");

        devServer.app.use(prefix, (req, res) => {
          ...
          proxyReq.on("error", (error) => {
            res.status(502).json({
              code: 502,
              msg: `代理${prefix}接口失败: ${error.message}`,
            });
          });
