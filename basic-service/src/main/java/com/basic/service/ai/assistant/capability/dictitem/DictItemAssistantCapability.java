package com.basic.service.ai.assistant.capability.dictitem;
...
import com.basic.service.ai.assistant.action.model.ActionPreviewField;
import com.basic.service.ai.assistant.capability.AssistantCapability;
import com.basic.service.ai.assistant.operation.AssistantOperationSpec;
import com.basic.service.sysDict.service.ISysDictService;
...
import static com.basic.service.ai.assistant.operation.AssistantOperations.*;
...
public class DictItemAssistantCapability implements AssistantCapability {
    ...
    private com.basic.service.ai.assistant.action.model.ActionExecutionResult create(DictItemSnapshot p){DictItemAddDTO d=new DictItemAddDTO();d.setDictId(p.dictId());d.setItemValue(p.itemValue());d.setItemLabel(p.itemLabel());d.setSort(p.sort());d.setStatus(p.status());Long id=itemService.addDictItem(d);return result(id,"字典项新增成功，字典项ID："+id);}
    private com.basic.service.ai.assistant.action.model.ActionExecutionResult update(DictItemSnapshot p){DictItemUpdateDTO d=new DictItemUpdateDTO();d.setId(p.id());d.setItemValue(p.itemValue());d.setItemLabel(p.itemLabel());d.setSort(p.sort());d.setStatus(p.status());itemService.updateDictItem(d,p.version());return result(p.id(),"字典项修改成功");}
    private com.basic.service.ai.assistant.action.model.ActionExecutionResult delete(DictItemSnapshot p){itemService.deleteDictItem(p.id(),p.version());return result(p.id(),"字典项删除成功");}
